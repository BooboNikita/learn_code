...
### 4.4 为什么要做 OPD 而不是直接混合 RL？

> 一句话：混合 RL 容易让不同领域的数据互相"打架"，OPD 先让每个领域登峰造极，再把这些专长「翻译」成统一模型的能力。

| 问题 | 混合 RL 的做法 | OPD 的解法 |
|------|---------------|-----------|
| 领域冲突 | 数学/代码/对话/长文本数据混在一起做 RL，优化一个方向会损害另一个方向 | 每个领域独立训练专家，互不干扰 |
| 专长稀释 | 统一模型被迫取平均，单领域顶尖能力被拉平 | 专家在各自领域做到极致，再合并 |
| 合并困难 | 直接合并专家权重或简单混合数据，会丢失领域细节 | 用 on-policy 采样 + reverse KL 精确保留专家分布 |
| 训练稳定 | RL 奖励信号稀疏，长链推理容易学偏 | 专家提供稳定的 full-logit 监督，梯度更稳 |
| 可控性 | 纯 RL 像"放养"，行为难预测 | OPD 像"名师授课"，合并过程可监控 |

**打个比方**：
- 混合 RL = 让一个学生同时跟 10 个老师学 10 门课，老师们同时说话，学生听混了
- OPD = 先让 10 个学生分别师从 10 位名师，再把这 10 个学生的知识「蒸馏」给一个全能学生

所以 V4 不是简单蒸馏一个小模型，而是**用蒸馏做「能力合并」**，把多个领域专家的顶尖能力集成到一个模型里。
