...
| 上下文长度 | **1M tokens（默认）** | **1M tokens（默认）** |
...
| 新特性 | 作用 |
|--------|------|
| **CSA + HCA 混合注意力** | 压缩稀疏注意力 + 高压缩注意力结合，解决长上下文效率瓶颈 |
| **mHC（流形约束超连接）** | 增强传统残差连接，提升模型表达能力 |
| **Muon 优化器** | 更快收敛、更高训练稳定性 |
| **MegaMoE** | 细粒度专家并行，通信与计算重叠，GPU/NPU 上加速 1.50~1.73 倍 |
| **FP4 量化** | MoE 专家权重 + CSA 中 QK 路径用 MXFP4，降低显存并加速长上下文 |
| **MTP 保留** | 继续沿用多 token 预测 |
...
1. **专家训练**：每个目标领域训练一个独立专家
   - 先用高质量领域数据做 **SFT**
   - 再用 **GRPO** 做领域强化学习
   - 得到十多位各有所长的「偏科高手」

2. **学生模型合并**：统一学生模型自己采样答题
   - 由专家老师在**完整词表 logit 层面**打分对齐
   - 用 **reverse KL 损失** 把学生拉向老师
   - 坚持**全词表蒸馏**，梯度更稳、训练曲线更可控

**所以 GRPO 还用吗？**
- **用**，但只在 OPD 第一阶段的「专家训练」里用
- V4 最终模型的主训练算法是 **OPD**，不是直接 GRPO
- 相比 R1 的纯 GRPO，V4 的后训练更强调**可控蒸馏**而非自由探索

### 4.4 推理历史跨轮次保留

在**工具调用 / Agent 场景**中，V4 会跨所有轮次保留完整推理历史（V3.2 每轮丢弃上一轮的思考轨迹），让长周期 Agent 任务能持续累积思考链条。

在**普通对话场景**中，仍按原策略丢弃上一轮推理内容以保持上下文简洁。
...
| 训练目标 | 下一个 token | + MTP | RL | OPD（专家内用 GRPO） |
| 推理特长 | 通用 | 通用 | **数学/代码/STEM** | 通用 + 长上下文 + Agent |
...
10. **V4 最显性的升级？** 原生 1M token 上下文、CSA+HCA 混合注意力、MegaMoE、FP4 量化、OPD 后训练。
11. **V4 还用了 GRPO 吗？** 用了，但只在 OPD 第一阶段的专家训练里；最终合并阶段是 OPD。
12. **OPD 相比混合 RL 的优势？** 全词表蒸馏 + reverse KL，训练更可控，各领域专长保留更完整。
13. **FlashMLA / DeepGEMM / DeepEP 分别优化什么？** 注意力 kernel、矩阵运算 kernel、MoE 专家并行通信。
14. **DeepSeek 为什么训练/推理成本低？** MLA + MoE 稀疏激活 + 无辅助损失 + kernel 优化 + MTP 数据效率。
